...
class pla:
    def __init__(self, name, path):
        self.name = name
        self.path1 = path + "txt" + os.path.sep
        self.path2 = path + "tfc" + os.path.sep
        self.top = ""
        self.c = ""
        self.ends = ""
        self.pControl = []
    ...
    def getGates(self):
        fileName = self.path1 + self.name + '.txt'
        with open(fileName, 'r') as f:
            ...
            while c.count('end') != 1:
                ...
                # if vargates[0] == 't1' and not vargates[1].count('f'):
    ...
    def writeTfc(self):
        fileName = self.path2 + self.name + '.tfc'
...
if __name__ == '__main__':
    for i in range(1, 4):
        path = os.path.dirname(os.path.abspath(__file__)) + os.path.sep + str(i) + os.path.sep
